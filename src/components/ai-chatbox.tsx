...
function localRAG(userMsg: string): string | null {
  ...
  if (q.includes('reviewradar') || q.includes('review radar')) {
    return "**ReviewRadar AI** is an end-to-end Yelp review intelligence platform (PostgreSQL ETL + VADER features + ensemble ML).\n\nPublished metrics from `model_metrics.json`: **enhanced_sentiment_ensemble_v4** reaches **accuracy 0.9106**, **F1 0.9341**, **ROC-AUC 0.9687** — vs ~0.84 accuracy on the v3.4/v3.5 pipelines.\n\n🔗 github.com/Thanuka9/reviewradar_ai";
  }
  if (q.includes('monday') && (q.includes('audit') || q.includes('graphql') || q.includes('board'))) {
    return "**Monday.com Data Auditor** is a local **Streamlit** tool that scans Monday boards via the **GraphQL API** with cursor pagination, flags missing Files/Notes cells, and exports CSV reports with deep links to each task.\n\n🔗 github.com/Thanuka9/Monday";
  }
  ...
  if (q.includes('central bank') || q.includes('cbsl') || q.includes('macroeconomic')) {
    return "Thanuka is a **Data Scientist at the Central Bank of Sri Lanka** (since Aug 2026).\n\nThe work is confidential, so only the general disciplines can be shared:\n• **Macroeconomic modelling** and time-series forecasting methodology\n• **Statistical rigour** — stationarity checks, residual diagnostics, and honest out-of-sample evaluation\n• **Financial sector analytics** and reproducible data pipelines\n• **Research communication** for a policy audience\n\nNo project data, figures, or outputs from this role are published anywhere on this site.";
  }
